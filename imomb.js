...
Solitaire.Index = new Ext.Controller({
    ...
    index : function() {
        ...
        this.fub  = view.get('fub');

        // Keep the fub short
        // from experimenting, I think these can only be adjusted
        // in terms of a fraction of card height--i.e. multiples of 6
        this.fub.layout.topOffset = 6;
        this.fub.layout.leftOffset = 0;
        this.deal.layout.topOffset = 0;
        this.deal.layout.leftOffset = 0;
        
        this.deck.on('draw', function() {
          console.log('draw');
          if(!this.deck.getTopCard()) {
              this.replenishDeck();
          }
          else {
            this.dealTopThree();
            ...
            /*
              ...
              this.dealTopCard();
              this.dealTopCard.defer(1000, this);
              this.dealTopCard.defer(2000,this);
            }
            */
            //console.log(this);
                //console.log("replen");
            ...
            listeners: {
                ...
                datachanged: function(store, game) {
                    if (this.game) {                        
                        ...
                        this.updateScore(this.game.get('score'));
                    }
                }
            }
        });
        ...
        mgr.on({
            ...
            dropped: function(e, stack, cards, mover) {
                ...
                if(this.view.get('suitstack1').isComplete() && 
                   this.view.get('suitstack2').isComplete() && 
                   this.view.get('suitstack3').isComplete() && 
                   this.view.get('suitstack4').isComplete()) {

                   previous_score = this.game.get('score');
                   this.game.set('score', previous_score+500);
                   console.log("you won!");
                }
                else {
                  console.log("not yet..");
    ...
    startNewGame: function(revealTop) {
        ...
        /* KDR I think this is where the cards are dealt.  The second for loop just sets up the stack sizes for klondike  */
        for (i = 1; i <= stackCount; i++) {
            ...
            //for (j = 0; j < i; j++) {
                ...
                cards[index].position = 0;
                index++;
            //}
        }

        oldidx = index;
        // Put 13 cards in the face up blind
        for(k = index; k<(oldidx+13); k++) {
          cards[k].stack_id = this.fub.id;
          cards[k].position = k - oldidx;
          index++;
        }
        this.fub.revealTopCard.defer(1, this.fub);

        // Put one card in the suit stack and record its rank
        suitstack = view.get('suitstack1');
        cards[index].stack_id = suitstack.id;
        cards[index].position = 0;
        //cards[index].moveToStack({ newStack: suitstack, });
        suitstack.revealTopCard.defer(200, suitstack);
        //console.log("suitstack starter:");
        //console.log(cards[index]);
        this.sss = cards[index].value;
        index++;
        ...
        game.set('moves', 0);
        game.set('time', 0);
        game.set('score', previous_score-45);
        game.set('sss', this.sss);
    ...
    revealTopCards: function() {
        ...
        for (i = 1; i <= stackCount; i++) {
            ...
            stack.revealTopCard.defer(10 + (i*60), stack);
        }
    },
    ...
    loadGame: function(game) {
        ...
        this.updateScore(game.get('score'));
        this.sss = game.get('sss');
    },
    ...
    moveToStack: function(options) {
        ...
        /*
          ...
          console.log(newStack)
          console.log(newStack instanceof Solitaire.SuitStack);
          console.log("oldStack");
          console.log(oldStack);
          console.log(oldStack instanceof Solitaire.SuitStack);
          console.log("mover stack");
          console.log(this.mover.stack);
          */
        ...
        else if(!((newStack instanceof Solitaire.SuitStack) && (this.mover.oldStack instanceof Solitaire.SuitStack))) {
          deltaScore = 5;
        }
                ...
                if (options.saveGame) {
                    (function() {
                        var game  = me.game,
                            ...
                            score = game.get('score') + deltaScore;
    ...
    /**
     * @private
     * Moves the top *three* cards on the Deck to the Deal
     * @return {Boolean} True if a card could be dealt, false otherwise
     */
    dealTopThree: function() {
      for(i=0; i<3; i++) {
        var card = this.deck.getTopCard();
    ...
    updateScore: function(score) {
        var text = String.format("{0} {1}", '$', score);
            
        this.view.get('score').update(text);
    },
...
Solitaire.Stack = Ext.extend(Ext.Container, {
  /* {{{ */
    ...
    canDrop: function(mover) {
        ...
        if(card.stack_id === 'suitstack1' || card.stack_id === 'suitstack2' || card.stack_id === 'suitstack3' || card.stack_id === 'suitstack4') {
          return 0;
        }
        
        if (topCard) {
            // Card being placed (card) is next down sequentially, or we're putting a king (13) on an ace (1)
            var rightValue = ((card.value == (topCard.value - 1)) || (card.value === 13 && topCard.value === 1)) ,
                rightSuit  = card.isRed() != topCard.isRed(),
                sameCard   = card === topCard;
            
            return sameCard || (rightValue && rightSuit);
        } else {
          if(card.stack_id === 'fub') {
            return 1;
          }
          // Not from the face up blind; check to make sure the fub is empty
          view = Solitaire.Index.view;
          fub  = view.get('fub');
          fubtop = fub.getTopCard();

          if(!fubtop) {
            // If it's empty, we can only put cards with the same value as the suit stack starter-1
            if(card.value === Solitaire.Index.sss-1) {
              return 1;
            }
          }
          return 0;
          //console.log(fub);
          //console.log(Solitaire.Index.view);
          //console.log(Solitaire.Board);
            //king
    ...
    /**
     * Returns the card currently on the bottom of the stack, or null if the stack is empty
     * @return {Solitaire.Card|null} The card
     */
...
Solitaire.SuitStack = Ext.extend(Solitaire.Stack, {
  /* {{{ */
    ...
    canDrop: function(mover) {
        ...
        if (topCard) {
            // Card being placed (card) is next up sequentially, or we're putting an ace (1) on a king (13)
            var rightValue = ((card.value === (topCard.value + 1)) || (card.value === 1 && topCard.value === 13)),
            ...
            //ace
            console.log(Solitaire.Index.sss);
            console.log(this);
            return card.value === Solitaire.Index.sss;
        }
    },
        ...
        */
        // deck, deal, suit stacks
        {
            ...
            items: [
                {width: 12},
            // fub stack
                {xtype: 'stack', name: 'fub', width: 312},
                {flex: 1},
        ...
        // Stacks
        {
            ...
            height: 500,
            items: [
            // This width controls the space between the left edge and the
                {width: 12},
            ...
            tpl: new Ext.XTemplate(
                '<tpl for=".">',
                    '<div class="savedGame">',
                        ...
                        'Score: {score}, Moves: {moves}, time: {time}, complete: {[values.complete ? "yes" : "no"]}',
                    '</div>',
                '</tpl>',
                {
